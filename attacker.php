      ...
      <div class="main-content">
         <div class="page-content">
           <div class="container-fluid">            
            <div class="row">
           <?php
           $attacker = htmlspecialchars($_GET['name']);
           if ($attacker) {
           if ($attacker == '') {
           $attacker = 'none';
           }
           $attacker = $attacker; // dummy
           } else {
           $attacker = 'none';
           }
           $a = $a = $db->query("SELECT * FROM user WHERE alias = '$attacker'");
           $j = $a->fetchArray();
           $user = $j['user_id'];
           ?>
            ...
            <h5 class="h4 card-title mdi mdi-guy-fawkes-mask text-success mb-2"> Statistik</h5>
            </div>
            <div class="card-body">
            <center>
            ...
            <tr>
            <td><?php
            $q = $db->query("SELECT COUNT(*) FROM mirror WHERE mirror.defacer = '$user'");
            $row = $q->fetchArray();
            $totalarc = $row[0];
            echo $totalarc;
            ?></td>
            <td><?php
            $q = $db->query("SELECT COUNT(*) FROM mirror WHERE status = '0' and special = '0' AND mirror.defacer = '$user'");
            $row = $q->fetchArray();
            $totalarc = $row[0];
            echo $totalarc;
            ?></td>
            <td><?php
            $q = $db->query("SELECT COUNT(*) FROM mirror WHERE status = '1' AND mirror.defacer = '$user'");
            $row = $q->fetchArray();
            $totalarc = $row[0];
            echo $totalarc;
            ?></td>
            <td><?php
            $q = $db->query("SELECT COUNT(*) FROM mirror WHERE home = '1' AND mirror.defacer = '$user'");
            $row = $q->fetchArray();
            $totalhm = $row[0];
            echo $totalhm;
            ?></td>
            <td><?php
            $q = $db->query("SELECT COUNT(*) FROM mirror WHERE special = '1' AND mirror.defacer = '$user' AND status = '0'");
            $row = $q->fetchArray();
            $totalspc = $row[0];
            echo $totalspc;
            ?></td>
            ...
            <div class="card">
            <div class="card-header">
            <h5 class="h4 card-title mdi mdi-guy-fawkes-mask text-success mb-2"> Attacker</h5>
            </div>
            <div class="card-body">
            <center>
            <div class="table-responsive pt-3 table-striped">
            ...
            $sort = 20;
            $hal = $sort * $curpage;
            $sel = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.defacer = '$user'");
            $num = 0;
            while($rw = $sel->fetchArray()){$num++;}
            // End Nya Cok
            
            // Db Select
            $q = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.defacer = '$user'  ORDER BY date DESC LIMIT $hal, $sort");
            echo "<tr>";
            $i = 1;
            ...
            $defid = $row['defacer'];
            $teamid = $row['team'];
            echo "<td colspan=\"2\"><a href=\"attacker.php?name=$alias\">$username</a></td><td colspan=\"2\">$team</td><td colspan=\"3\">$date</td><td colspan=\"2\">$url</td><td><a href='ip.php?ip=$ip'>$mas</a></td><td>$home</td><td><img src=\"flags/$flag.png\" style=\"margin-top: 5px;\" /></td><td>$ip</td><td>$special</td><td><a href=\"notif.php?id=$mirrorid\"><i>$mirrorshow</i></td></tr>";
            ...
            $neg = $page - 1;
            $pos = $page + 1;
            if($page == 1){echo '<li class="page-item disabled" ><a class="page-link" disabled="disabled" href="#"><i class="material-icons">Previous</i></a></li>';}
            else{echo '<li class="page-item"><a class="page-link" href="?name='.$attacker.'&page='.$neg.'"><i class="material-icons">Previous</i></a></li>';}
            
            echo '<li class="page-item active"><a class="page-link" href="">'.$page.'</a></li>';
            
            if($page == $coun){echo '<li class="page-item disabled"><a class="page-link" disabled="disabled" onClick="page()"><i class="material-icons">Next</i></a></li>';}
            else{echo '<li class="page-item"><a class="page-link" href="?name='.$attacker.'&page='.$pos.'"><i class="material-icons">Next</i></a></li>';}
            ?>
            </ul>
