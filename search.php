                  ...
                  <div class="card-body">
                    <div class="d-flex">
                      ...
                      <div class="flex-shrink-0 align-self-center ">
                        ...
                        <h5 class="h4 card-title  text-success mb-2"> Search</h5>
                        </div>
                        <div class="card-body">
                        <center>
                        ...
                        $tid = $raw['team_id'];
                        $q = $q = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.team = '$tid'  ORDER BY date DESC LIMIT $hal, $sort");
                        ...
                        if ($rew['user_id']) {
                        // Config Pagnatin
                        $sel = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.team = '$teamid'  ORDER BY date DESC");
                        $num = 0;
                        while($rw = $sel->fetchArray()){$num++;}
                        // End Nya Cok
                        
                        $did = $rew['user_id'];
                        $q = $q = $db->query("SELECT * FROM mirror LEFT JOIN team ON mirror.team = team.team_id INNER JOIN user ON mirror.defacer = user.user_id WHERE mirror.defacer = '$did'  ORDER BY date DESC LIMIT $hal, $sort");
                        ...
                        $defid = $row['defacer'];
                        $teamid = $row['team'];
                        echo "<td colspan=\"2\"><a href=\"attacker.php?name=$alias\">$username</a></td><td colspan=\"2\">$team</td><td colspan=\"3\">$date</td><td colspan=\"2\">$url</td><td>$mas</td><td>$home</td><td><img src=\"flags/$flag.png\" style=\"margin-top: 5px;\" /></td><td>$ip</td><td>$special</td><td><a href=\"notif.php?id=$mirrorid\"><i>$mirrorshow</i></td></tr>";
                        ...
                        else{echo '<li class="page-item"><a class="page-link" href="?q='. $_REQUEST['q'].'&page='.$neg.'">Previous</a></li>';}
                        echo '<li class="page-item active"><a class="page-link" href="">'.$page.'</a></li>';
                        if($page == $coun){echo '<li class="page-item disabled"><a class="page-link" disabled="disabled" onclick="page()>Next</a></li>';}
                        else{echo '<li class="page-item"><a class="page-link" href="?q='. $_REQUEST['q'].'&page='.$pos.'">Next</a></li>';}
                        ?>
                        </ul>
                        </div> 
                        </div>
